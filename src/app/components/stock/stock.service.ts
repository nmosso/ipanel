...
export class StockService {
  ...
  async getDevicesinfo(clientid: number = null, allDevicesInfo: boolean, tenantid: string = null, masterstorage: boolean = false) {
    return new Promise(async (resolve, reject) => {
      let noclients = (allDevicesInfo == true) ? 'true' : 'false';

      let endPoint = ENDPOINTS.devices;
      let queryParams = `noclients=${noclients}&masterstorage=${masterstorage}`;
      if (clientid !== null) {
      ...
      endPoint = `${ENDPOINTS.devices}?${queryParams}`;
  ...
  async devicesUnassign(formData, tenantid = null) {

    return new Promise(async (resolve, reject) => {
      let endpoint = `${ENDPOINTS.devices}/unasssign/${formData.deviceid}`
      if (tenantid !== null) {
        endpoint = `${ENDPOINTS.devices}/unasssign/${formData.deviceid}/${tenantid}`
      }
